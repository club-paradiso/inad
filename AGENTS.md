# INAD: 제12조 — 작업 규칙 (Claude / Codex / Gemini 공용)

이 파일이 작업 규칙의 원본입니다. `CLAUDE.md`와 `GEMINI.md`는 이 파일을 가리키는
한 줄짜리 포인터이므로 따로 갱신할 필요가 없습니다. 워크스페이스 공통 규칙은
`~/.claude/CLAUDE.md`(= `~/.codex/AGENTS.md` = `~/.gemini/GEMINI.md`)에 있으며
여기서 되풀이하지 않습니다.
