...
- 런타임: HTML + CSS + Vanilla JS. 프레임워크·jQuery·Tailwind·외부 CDN·외부 폰트·외부 이미지·외부 오디오 금지. 런타임 네트워크 요청 0.
- 기본 UI 언어는 한국어. 영어는 INAD·PNR·K-ETA·MRZ·CLEAR·SECONDARY 등 약어와 여권 로마자에만 보조적으로 사용.
...
- 엔진 → UI 통지는 `src/js/services/bus.js` 이벤트(`changed`, `log`, `ops`, `toast`, `announce`, `sound`, `pulse`).
...
- 금지: 사이버펑크/네온/글로우, 의미 없는 그라디언트, 카드 안의 카드, 8px 이하 모노 텍스트 남발, 영어가 한국어보다 우선하는 시각 위계.
...
- `npm run lint` — 문법·엔진 DOM 접근·외부 URL·console.log 검사.
