...
export function renderBehavior() {
  ...
  const note = byId('behaviorNote'); if (note) note.textContent = `${b.profile.name} · ${band.response} · 최근 변화: ${b.lastEvent} · 이 정보는 법적 판단 점수에 사용되지 않습니다.`;
}
export function renderLanguage() {
  const l = state.language; if (!l) return;
  byId('primaryLang').textContent = l.primary; byId('koLevel').textContent = languageLabel(l.korean); byId('enLevel').textContent = languageLabel(l.english);
  const ko = byId('langKo'), en = byId('langEn'), it = byId('langInterp');
  ...
  const s = byId('languageStatus'); const direct = currentLanguageLevel(), need = 2;
